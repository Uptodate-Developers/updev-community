...
import {AddVoteRequest} from "../../requests/AddVoteRequest";
...
export class ReplyController {
    ...
    @Post("/vote")
    @Authorize(AuthProtocols.Jwt)
    async votePost(@BodyParams()voteRequest:AddVoteRequest,@Res() res){
        const reply = await this.replyService.addVote(voteRequest.itemId,voteRequest.userId,voteRequest.voteStatus)
        if(reply){
            res.status(StatusCodes.Created)
            return serialize(reply)
        }
