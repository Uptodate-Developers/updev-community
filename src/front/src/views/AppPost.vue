<template>
  ...
  <div class="space-y-2 md:hidden">
    <div v-if="!isLoading" class="space-y-4">
      ...
      <PopularTopics sectionTitle="Autres posts" :tags="tags" :exceptPost="post" />
    </div>
  </div>
  <div v-if="!isLoading" class="hidden container mx-auto md:grid grid-cols-9 gap-2 py-2 xl:px-28">
    <div class=" col-span-6">
      <ForumPost :post="post" :user="user"/>
    </div>
    <div class=" space-y-3 col-span-3 md:sticky top-16 h-screen z-20 overflow-y-auto hiddescrollbar">
      <UserProfile :user="postUser" />
      <PopularTopics sectionTitle="Autres posts" :tags="tags" :exceptPost="post" />
    </div>
  </div>
...
export default defineComponent({
  ...
  setup(props,context) {
    ...
    watch(() => props.id,async () => await onLoadPost())

    return {key,post,user,postUser,tags}
  }
})
</script>
<style scoped>
