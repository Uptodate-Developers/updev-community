<template>
    <div class=" bg-gray-50 px-2 py-2 rounded-md">
      ...
      <div class="grid grid-cols-8 lg:grid-cols-10">
        <a-avatar :src="user.profilePicUrl" :style="{backgroundColor: '#0068B7', verticalAlign: 'middle'}" :size="40"><span style="line-height: 40px" class="block text-xl font-semibold">{{ avatar }}</span></a-avatar>
        <new-post-input :user="user" class="col-span-7 lg:col-span-9"/>
      </div>
    </div>
</template>
<script lang="ts">
