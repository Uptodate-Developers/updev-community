  ...
  <div class="relative mb-72">
    ...
    <div class="flex flex-wrap space-x-1 space-y-1 px-4 py-2 bg-gray-50">
      <button
        ...
        :key="hashTag.tag"
        @click="onSelectTag(hashTag)"
        :class="
          hashTag.selected
            ? 'bg-blue-500 text-gray-50'
            : 'bg-gray-200 text-gray-500'
        "
        class="px-4 pb-0.5 rounded-2xl text-xs focus:outline-none"
      >
        #{{ hashTag.tag }}
      </button>
    </div>
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const onLoadTags = async () => {
      ...
      if (typeof tags == "string") message.error(tags);
      else
        hashTags.value = hashTagService.checkSelectedTags(
          tags,
