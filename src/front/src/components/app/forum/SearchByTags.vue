<template>
  <a-list v-if="showPosts"   :data-source="posts" :grid="{ column: 1 }" itemLayout="vertical">
    <template #loadMore>
...
export default defineComponent({
  ...
  setup(props){
    ...
    const onPosts = async (tags:string[]) => {
      ...
      const postsResponse = await postService.getPostsForTags(tags,isPopular.value,skip.value,take.value)
    ...
    watch(() => props.tags,async () => await getPosts())

    onMounted(async () => await getPosts())


    return{showPosts,posts,canLoadMore,loadMore}
  }
})
</script>
