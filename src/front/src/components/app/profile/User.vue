<template>
  <div v-if="user" class="px-4 py-4 bg-gray-50">
    <div class="flex justify-between">
      ...
      >
        <svg
          class="fill-current hover:text-blue-400"
          id="edit"
    ...
    <div class="mt-3 md:flex justify-between items-center md:space-x-8">
      ...
      <div class="space-x-2 flex">
        <button
          ...
          @click="onOpenSocial(user.githubProfileLink)"
        >
          <img
            class="h-8"
            src="../../../assets/icons/githubblack.svg"
            alt="Github logo"
          />
        </button>
        <button
          class="focus:outline-none"
          @click="onOpenSocial(user.twitterProfileLink)"
        >
          <img
            class="h-8"
        ...
        <button
          class="focus:outline-none"
          @click="onOpenSocial(user.facebookProfileLink)"
        >
          <img
            class="h-8"
...
export default defineComponent({
  ...
  setup(props) {
    ...
    watch(
      () => props.user,
      () => (isOwner.value = authService.user?.id == user.value.id)
    );
