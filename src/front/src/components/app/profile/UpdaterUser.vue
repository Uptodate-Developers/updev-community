  ...
  >
    ...
    <a-form :model="userForm" layout="vertical">
      ...
      <a-form-item label="Profession">
        <a-input
          v-model:value="userForm.profession"
...
import BaseImagePicker from "./../../form/BaseImagePicker.vue";

export default defineComponent({
  name: "UpdateUser",
  components: { LoadingOutlined, PlusOutlined, BaseImagePicker },
  props: {
    user: {
      type: Object as () => User,
      required: true,
  ...
  setup(props) {
    ...
    const onSubmit = async () => {
      ...
      userForm.profilePicUrl = uploadImageUrl
        ? uploadImageUrl
    ...
    const handleChange = async (file: File) => {
      if (file) {
        isLoading.value = true;
        const uplaodResult = await photoService.uploadUserPorfilePic(file);
